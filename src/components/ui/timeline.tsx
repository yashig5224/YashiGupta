    ...
    >
      {(heading || subheading) && (
        <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
          {heading && (
            <h2 className="text-lg md:text-4xl mb-4 text-foreground font-display font-bold max-w-4xl">
              {heading}
            </h2>
          )}
          {subheading && (
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-xl md:pl-20 md:text-5xl font-bold text-muted-foreground/50 font-display">
                {item.title}
              </h3>
            </div>
